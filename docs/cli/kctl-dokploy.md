...
Command reference for `kctl-dokploy` (48 groups, ~264 commands).
...
| `compose update <compose_id> [--env_content] [--compose_file] [--source_type] [--repository] [--owner] [--branch] [--compose_path] [--github_id] [--command] [--trigger_type] [--watch_paths] [--enable_submodules] [--auto_deploy]` | Update a compose service configuration. |
...
| Command | Description |
|---------|-------------|
| `deploy apply <file> [--dry_run] [--skip_deploy] [--skip_verify]` | All-in-one: setup + deploy + post-deploy in sequence. |
...
| `docker containers [--server_id]` | List Docker containers. |
...
| Command | Description |
|---------|-------------|
| `monitoring container-logs <container_name> [--tail]` | Show logs from a Docker container. |
