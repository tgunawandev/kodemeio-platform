...
Scans deploys/env/*.env files, groups them by base type (odoo, react-pwa,
...
Usage:
    ...
    uv run python scripts/check-env-parity.py --json
"""
...
def discover_env_groups() -> dict[str, list[tuple[str, Path]]]:
    ...
    for env_path in sorted(ENV_DIR.glob("*.env")):
        stem = env_path.stem  # e.g., "kodeme.io-odoo-full"
        # Find matching instance manifest
        manifest_path = INSTANCES_DIR / f"{stem}.yaml"
        ...
        groups[base_type].append((stem, env_path))
            ...
            for d in drift:
                ...
                for val, insts in value_groups.items():
                    if len(insts) <= 3:
                        ...
                        inst_str = (
                            f"{insts[0]}, {insts[1]}, ... (+{len(insts) - 2} others)"
                        )
        ...
        if missing:
            ...
            for m in missing:
                ...
                if len(missing_from) <= 3:
                    ...
                    missing_str = (
                        f"{missing_from[0]}, {missing_from[1]}, "
                        f"... (+{len(missing_from) - 2} others)"
                    )
                print(f"    {key}")
            ...
            masked_drift.append(
                {
                    "key": d["key"],
                    "values": {
                        inst: mask_value(d["key"], val)
                        for inst, val in d["values"].items()
                    },
                }
            )
...
def main() -> int:
    ...
    if args.json_output:
        ...
        print()
        print(
            f"Summary: {sum(len(v) for v in groups.values())} instances "
            f"across {len(groups)} types"
        )
