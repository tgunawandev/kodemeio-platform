"""Doctor diagnostic checks for kctl-hz."""
...
class APIConnectivityCheck:
    """Check that the Hetzner API is configured."""

    name: str = "API Connectivity"

    def run(self) -> CheckResult:
        try:
            from kctl_hz.core.config import get_service_config, resolve_active_profile_name

            profile = resolve_active_profile_name()
            cfg = get_service_config(profile)
            location = cfg.default_location or ""
            if not cfg.token:
                return CheckResult(
                    name=self.name,
                    status="fail",
                    message="No API token configured",
                    fix_command="kctl-hz config init",
                )
            return CheckResult(name=self.name, status="ok", message=f"Default location: {location}")
