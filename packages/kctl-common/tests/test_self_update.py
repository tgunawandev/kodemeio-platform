"""Tests for kctl_common.self_update."""
...
from kctl_common.self_update import check_update, update
...
class TestCheckUpdate:
    def test_newer_version_returns_version(self) -> None:
        ...
        with patch("kctl_common.self_update.run_quiet", return_value=_make_result(pip_output)):
            result = check_update("kctl-common", "0.2.0")

        assert result == "0.3.0"

    def test_same_version_returns_none(self) -> None:
        pip_output = "kctl-common 0.2.0\n"
        with patch("kctl_common.self_update.run_quiet", return_value=_make_result(pip_output)):
            result = check_update("kctl-common", "0.2.0")
    ...
    def test_command_failure_returns_none(self) -> None:
        with patch("kctl_common.self_update.run_quiet", return_value=_make_result("", returncode=1)):
            result = check_update("kctl-common", "0.1.0")

        assert result is None

    def test_empty_output_returns_none(self) -> None:
        with patch("kctl_common.self_update.run_quiet", return_value=_make_result("")):
            result = check_update("kctl-common", "0.1.0")
    ...
    def test_version_with_parenthesis_line_skipped(self) -> None:
        ...
        with patch("kctl_common.self_update.run_quiet", return_value=_make_result(pip_output)):
            result = check_update("kctl-common", "0.3.0")

        assert result == "0.5.0"

    def test_invalid_version_format_returns_none(self) -> None:
        pip_output = "kctl-common v0.3.0-beta\n"
        with patch("kctl_common.self_update.run_quiet", return_value=_make_result(pip_output)):
            result = check_update("kctl-common", "0.2.0")
...
class TestUpdate:
    def test_calls_uv_tool_upgrade(self) -> None:
        ...
        with patch("kctl_common.self_update.run", mock_run):
            update("kctl-common")

        mock_run.assert_called_once()
        cmd = mock_run.call_args[0][0]
        assert cmd == ["uv", "tool", "upgrade", "kctl-common"]
