...
Usage:
    ...
    uv run python scripts/inventory.py --format csv --output inventory.csv
"""
...
def format_markdown(inv: Inventory) -> str:
    ...
    for project, instances in inv.group_by_project().items():
        ...
        lines.append("")
        rows = [
            [i.name, i.type, i.domain or "(no domain)", i.server] for i in instances
        ]
    ...
    lines.append(
        _md_table(["Instance", "Has Backup", "Schedule", "Retention"], backup_rows)
    )
    lines.append("")
...
def format_json(inv: Inventory) -> str:
    ...
    payload = {
        ...
        "by_project": {
            project: [asdict(i) for i in instances]
            for project, instances in inv.group_by_project().items()
        },
    }
