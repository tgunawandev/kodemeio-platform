...
#
# Usage:
#   ./monitoring/scripts/apply-monitoring.sh              # Apply everything
#   ./monitoring/scripts/apply-monitoring.sh --dry-run    # Preview only
#   ./monitoring/scripts/apply-monitoring.sh --component grafana  # Single component
#
# Prerequisites:
#   - kctl-gatus, kctl-grafana, kctl-glitchtip configured with active profiles
#   - Verify with: kctl-gatus config current / kctl-grafana config current
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --component)
            ...
            ;;
        -h|--help)
            echo "Usage: $0 [--dry-run] [--component gatus|grafana|alerts|all]"
            exit 0
            ;;
        *)
...
apply_gatus() {
    ...
    kctl-gatus endpoints list --format pretty
    ...
    log_info "  2. Restart Gatus: kctl-dokploy compose redeploy kodeme.io-infra-gatus"
    log_info ""
    log_info "Verifying Gatus health..."
    kctl-gatus health check

    log_info "Gatus endpoint count:"
    kctl-gatus endpoints list --json | python3 -c "import sys,json; data=json.load(sys.stdin); print(f'  {len(data)} endpoints configured')" 2>/dev/null || log_warn "Could not count endpoints"
}
...
apply_grafana() {
    ...
    for dashboard_file in "$DASHBOARD_DIR"/*.json; do
        ...
        kctl-grafana dashboard import "$dashboard_file" --overwrite
...
apply_glitchtip() {
    ...
    kctl-glitchtip projects list 2>/dev/null || log_warn "Could not list GlitchTip projects (check config)"

    log_info "GlitchTip health:"
    kctl-glitchtip health check 2>/dev/null || log_warn "Could not reach GlitchTip"
}
