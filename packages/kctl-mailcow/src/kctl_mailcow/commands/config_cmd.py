...
from kctl_mailcow.core.config import (
    CONFIG_FILE,
    SERVICE_KEY,
    ServiceConfig,
...
def _test_connection(url: str, api_key: str) -> tuple[bool, str]:
    try:
        client = MailcowClient(base_url=url, credential=api_key)
        ok, info = client.check_health()
        client.close()
        return ok, info
...
def init(
    ctx: typer.Context,
    url: Annotated[str | None, typer.Option("--url", help="Mailcow base URL.")] = None,
    ...
    api_url = url or typer.prompt("Mailcow URL (e.g. https://mail.kodeme.io)")
    ...
    if ok:
        out.success(f"Connected to Mailcow ({info})")
    else:
...
def add(
    ...
    """Add or update a profile's Mailcow connection."""
    actx: AppContext = ctx.obj
    out = actx.output

    api_url = url or typer.prompt("Mailcow URL")
    key = api_key or typer.prompt("API key", hide_input=True)
...
def remove(
    ...
    service_only: Annotated[
        bool, typer.Option("--service-only", help="Only remove mailcow config, keep other services")
    ] = False,
) -> None:
    """Remove a profile or just its Mailcow config."""
    actx: AppContext = ctx.obj
    out = actx.output
...
def show(ctx: typer.Context) -> None:
    ...
    sections.append(
        (
            ...
            [
                ...
                ("This CLI", f"kctl-mailcow -> service key: {SERVICE_KEY}"),
            ],
        )
    )
    ...
    for pname in profiles:
        ...
        for svc_name, svc_data in services.items():
            ...
            indicator = "[green]o[/green]" if svc_name == SERVICE_KEY else "[dim]o[/dim]"
...
def profiles(ctx: typer.Context) -> None:
    """List all profiles with Mailcow connection status."""
    actx: AppContext = ctx.obj
    out = actx.output

    profile_names = get_profile_names()
    if not profile_names:
        out.warn("No profiles configured. Run: kctl-mailcow config init")
        return
    ...
    for pname in profile_names:
        ...
        if svc.url:
            ok, info = _test_connection(svc.url, svc.api_key)
            conn_status = "[green]connected[/green]" if ok else "[red]offline[/red]"
        else:
            conn_status = "[dim]no mailcow config[/dim]"
    ...
    out.table(
        "Profiles",
        [("Name", "cyan"), ("Mailcow URL", ""), ("Status", ""), ("Other Services", "dim"), ("", "green")],
        rows,
...
def current(ctx: typer.Context) -> None:
    ...
    if actx.profile:
        source = "--profile flag"
    elif os.environ.get("KCTL_MAILCOW_PROFILE"):
        source = "KCTL_MAILCOW_PROFILE env var"
    ...
    if all_profiles:
        sections.append(("Other Profiles", [(p, get_service_config(p).url or "(no mailcow)") for p in all_profiles]))
...
def test(ctx: typer.Context) -> None:
    ...
    out.detail(
        ...
        data_for_json={"profile": active, "service": SERVICE_KEY, "connected": True},
    )
