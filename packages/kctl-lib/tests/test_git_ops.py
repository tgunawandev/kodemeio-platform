...
from kctl_lib.git_ops import branch_status, changelog_generate, diff_summary, pr_create, release_tag
...
class TestBranchStatus:
    def test_clean_branch_no_ahead_behind(self) -> None:
        ...
        with patch("kctl_lib.git_ops.run_quiet", side_effect=side_effect):
...
class TestDiffSummary:
    def test_typical_diff(self) -> None:
        ...
        with patch("kctl_lib.git_ops.run_quiet", return_value=_make_result(raw)):
            result = diff_summary()
    ...
    def test_git_failure_returns_zeros(self) -> None:
        with patch("kctl_lib.git_ops.run_quiet", return_value=_make_result("", returncode=1)):
            result = diff_summary()

        assert result == {"files_changed": 0, "insertions": 0, "deletions": 0, "raw": ""}

    def test_empty_diff(self) -> None:
        with patch("kctl_lib.git_ops.run_quiet", return_value=_make_result("")):
            result = diff_summary()
...
class TestChangelogGenerate:
    def test_mixed_commits(self) -> None:
        ...
        with patch("kctl_lib.git_ops.run_quiet", return_value=_make_result(log)):
    ...
    def test_since_tag_passed_to_git(self) -> None:
        with patch("kctl_lib.git_ops.run_quiet", return_value=_make_result("")) as mock_rq:
...
class TestPrCreate:
    def test_calls_gh_pr_create(self) -> None:
        ...
        with patch("kctl_lib.git_ops.run", mock_run):
