"""Doctor diagnostic checks for kctl-redis."""
...
class SSHCheck:
    ...
    def run(self) -> CheckResult:
        try:
            from kctl_redis.core.config import get_service_config, resolve_active_profile_name
            ...
            if not host:
                return CheckResult(
                    ...
                    fix_command="kctl-redis config init",
                )
        ...
        try:
            ...
            if not cfg.host:
                return CheckResult(
                    name=self.name,
                    status="warn",
                    message="No Redis host configured",
                    fix_command="kctl-redis config init",
                )
