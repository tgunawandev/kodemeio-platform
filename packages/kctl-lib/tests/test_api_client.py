...
class TestConstructor:
    def test_requires_credential(self) -> None:
        with pytest.raises(ConfigError):
            SampleClient(base_url="https://example.com", credential="")

    def test_requires_base_url_when_no_class_base_url(self) -> None:
        with pytest.raises(ConfigError):
            SampleClient(credential="my-key")
    ...
    def test_api_prefix_appended(self) -> None:
        c = SampleClient(base_url="https://example.com", credential="key")
        assert c._base_url.endswith("/api/v1")

    def test_api_prefix_not_doubled(self) -> None:
        c = SampleClient(base_url="https://example.com/api/v1", credential="key")
    ...
    def test_context_manager(self) -> None:
        with SampleClient(base_url="https://example.com", credential="key") as c:
            assert isinstance(c, SampleClient)
...
class TestAuthHeaders:
    def test_raw_key_no_prefix(self) -> None:
        c = SampleClient(base_url="https://example.com", credential="my-key")
