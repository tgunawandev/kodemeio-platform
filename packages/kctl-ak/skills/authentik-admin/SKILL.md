...
description: >
  Authentik identity provider administration via kctl-ak CLI (28 groups, ~198 commands).
  MUST use for ANY kctl-ak operation.
  Triggers on: "access", "activate", "active", "add-user", "admin", "apply", "apps", "audit", "batch", "bind", "bindings", "blueprints", "brands", "bulk-import", "bulk-invite", "bulk-recovery-links", "cache-clear", "certificates", "changes", "check", "check-access", "clean", "completions", "config", "connections", "create-authenticator-validate", "create-connection", "create-consent", "create-email", "create-identification", "create-ldap", "create-password", "create-prompt", "create-recovery", "create-rule", "create-saml", "create-scim", "create-scope", "create-transport", "create-user-login", "create-user-logout", "current", "dashboard", "deactivate", "delete-connection", "delete-impersonation", "delete-rule", "delete-transport", "doctor", "event-retention", "execute", "expire-all", "export", "flows", "generate", "groups", "health", "impersonation", "init", "instances", "invite", "kctl-ak", "kill", "kill-user", "launch-urls", "ldap", "license", "logins", "mail", "maintenance", "mark-read", "members", "migrate", "notifications", "oauth2", "offboard", "onboard", "orphaned", "outposts", "password".
  Auto-generated: 2026-04-14
  registry_hash: 6fa3fd9e4ae4
---

# authentik-admin — kctl-ak CLI Reference

> Auto-generated from `kctl-ak` command registry. Do not edit manually.
> To regenerate: `kctl-ak skill generate`
...
| `apps check-access <slug> [--user]` | Check whether a user can access an application. |
...
| `apps list [--only_accessible]` | List all applications. |
| `apps orphaned` | List applications that have no active provider. |
| `apps set-icon <slug> <source>` | Set application icon from a local file or URL. |
| `apps sync [--dry_run] [--prune] [--file]` | Sync applications from app-registry.yaml (3-phase: create/update/prune). |
...
| `brands update <id_> [--name] [--branding_title] [--branding_logo] [--branding_favicon] [--flow_authentication] [--flow_invalidation] [--flow_recovery]` | Update a brand. |
...
Usage: `kctl-ak completions [--shell] [--install]`
...
| `flows create-recovery [--slug] [--name] [--title] [--set_on_brand]` | Create a minimal recovery flow (prompt + write + login) and optionally bind to a brand. |
...
| `groups sync [--dry_run] [--prune] [--file]` | Sync groups from group-structure.yaml (3-phase: create/update/prune). |
...
Cross-system user provisioning (Authentik + Mailcow + Odoo).

| Command | Description |
|---------|-------------|
| `provision offboard <email> [--dry_run]` | Disable user across Authentik, Mailcow, and Odoo. |
| `provision onboard <email> [--name] [--company] [--dry_run]` | Provision user across Authentik, Mailcow, and Odoo. |
| `provision setup-webhook [--company] [--all_companies] [--check] [--remove] [--ak_sync_url]` | Setup Odoo HRMS automated action to fire webhooks to ak-sync. |
| `provision status <email>` | Check user status across all systems. |
| `provision sync [--company] [--all_companies] [--dry_run]` | Poll HRMS and reconcile users across systems. |
...
| `setup batch [--dry_run] [--output_env] [--file]` | Bulk create OAuth2 providers for all apps in app-registry.yaml. |
...
**Examples:**
```bash
kctl-ak skill generate
kctl-ak skill generate --install
kctl-ak skill generate --check
```
...
| `users bulk-import <file> [--groups] [--skip_existing]` | Bulk-import users from a JSON or CSV file. |
| `users bulk-invite <file> [--send_mail]` | Bulk invite users from a JSON file. |
| `users bulk-recovery-links <file> [--out] [--outline_collection] [--app_install_urls]` | Generate one-time recovery links for a list of users. |
...
```bash
kctl-ak config init       # Interactive setup
kctl-ak config show       # Show current config
kctl-ak config profiles   # List profiles
kctl-ak config current    # Show active profile
kctl-ak config validate   # Verify config
```
