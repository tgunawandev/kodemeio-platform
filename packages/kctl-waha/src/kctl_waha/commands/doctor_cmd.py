"""Doctor diagnostic checks for kctl-waha."""
...
class ConfigCheck:
    ...
    def run(self) -> CheckResult:
        try:
            from kctl_waha.core.config import get_service_config, resolve_active_profile_name
            ...
            if not cfg.url:
                return CheckResult(
                    name=self.name,
                    status="fail",
                    message="No WAHA URL configured",
                    fix_command="kctl-waha config init",
                )
            return CheckResult(name=self.name, status="ok", message=f"Profile: {profile}, URL: {cfg.url}")
