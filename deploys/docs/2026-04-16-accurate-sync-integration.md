...
**Scope:** `kodemeio-platform/deploys/` — add first-class support for `kodemeio-accurate-sync` so tenants can declare it with one flag and the generator emits the right env file.
...
3. Review diff, commit to the 18.0 branch of kodemeio-platform.
...
Option (a) would copy `compose/accurate-sync.yml` into `kodemeio-odoo/compose/` or bake the compose content into `instances/production/tpp-accurate-sync.yaml` itself so Dokploy could pull it from kodemeio-platform.

Rejected because:
- **Drift surface.** The image and the compose file evolve together in `kodemeio-accurate`. Splitting them across two repos means any image change (env var addition, volume mount, healthcheck tweak) needs a matching commit in kodemeio-platform, and the two can get out of sync silently.
