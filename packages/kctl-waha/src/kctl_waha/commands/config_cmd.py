...
from kctl_waha.core.client import WahaClient
from kctl_waha.core.config import (
    CONFIG_FILE,
    SERVICE_KEY,
    ServiceConfig,
...
def _test_connection(url: str, api_key: str) -> tuple[bool, str]:
    try:
        client = WahaClient(base_url=url, api_key=api_key)
        health = client.check_health()
        client.close()
        if health:
            return True, health.get("version", "connected")
        return False, "Health endpoint returned empty response"
...
def init(
    ctx: typer.Context,
    url: Annotated[str | None, typer.Option("--url", help="WAHA base URL.")] = None,
    api_key: Annotated[str | None, typer.Option("--api-key", help="API key.")] = None,
    bridge_url: Annotated[str | None, typer.Option("--bridge-url", help="Bridge sidecar URL.")] = None,
    ...
    api_url = url or typer.prompt("WAHA URL (e.g. https://waha.kodeme.io)")
    ...
    if ok:
        out.success(f"Connected to WAHA ({version})")
    else:
...
def add(
    ...
    """Add or update a profile's WAHA connection.

    Example: kctl-waha config add production --url https://waha.kodeme.io --api-key $KEY

    This writes to the 'waha' section within the profile, so other
    kctl-* tools (kctl-odoo, kctl-ak) can coexist in the same profile.
    """
    actx: AppContext = ctx.obj
    out = actx.output

    api_url = url or typer.prompt("WAHA URL")
    key = api_key or typer.prompt("API key", hide_input=True)
    ...
    # Check if this profile already has waha config
...
def use(
    ...
    Example: kctl-waha config use production
    """
    actx: AppContext = ctx.obj
    out = actx.output
    ...
    if svc.url:
        ...
        if ok:
            out.success(f"Switched to '{name}' ({svc.url}) -- WAHA {version}")
        else:
...
def remove(
    ...
    service_only: Annotated[
        bool, typer.Option("--service-only", help="Only remove waha config, keep other services")
    ] = False,
) -> None:
    """Remove a profile or just its WAHA config."""
    actx: AppContext = ctx.obj
    out = actx.output
    ...
    sections.append(
        (
            ...
            [
                ...
                ("This CLI", f"kctl-waha -> service key: {SERVICE_KEY}"),
            ],
        )
    )
    ...
    for pname in profiles:
        ...
        for svc_name, svc_data in services.items():
            ...
            if svc_name == SERVICE_KEY:
                if svc_data.get("bridge_url"):
                    kvs.append(("  bridge_url", svc_data["bridge_url"]))
...
def set_(
    ctx: typer.Context,
    key: Annotated[str, typer.Argument(help="Config key (e.g. url, api_key, bridge_url, or default_profile)")],
    ...
    valid_fields = {"url", "api_key", "bridge_url", "container_name"}
...
def profiles(ctx: typer.Context) -> None:
    """List all profiles with WAHA connection status."""
    actx: AppContext = ctx.obj
    out = actx.output

    profile_names = get_profile_names()
    if not profile_names:
        out.warn("No profiles configured. Run: kctl-waha config init")
        return
    ...
    for pname in profile_names:
        ...
        if svc.url:
            ...
            conn_status = "[dim]no waha config[/dim]"
    ...
    out.table(
        "Profiles",
        [("Name", "cyan"), ("WAHA URL", ""), ("Status", ""), ("Other Services", "dim"), ("", "green")],
        rows,
...
def current(ctx: typer.Context) -> None:
    ...
    if actx.profile:
        source = "--profile flag"
    elif os.environ.get("KCTL_WAHA_PROFILE"):
        source = "KCTL_WAHA_PROFILE env var"
    ...
    sections: list[tuple[str, list[tuple[str, str]]]] = [
        (
            ...
            [
                ...
                ("Status", f"[green]Connected -- WAHA {version}[/green]" if ok else f"[red]{version}[/red]"),
            ],
        ),
    ]
    ...
    if svc.bridge_url:
        extras.append(("Bridge URL", svc.bridge_url))
    if svc.container_name:
        extras.append(("Container", svc.container_name))
    if extras:
    ...
    if all_profiles:
        sections.append(("Other Profiles", [(p, get_service_config(p).url or "(no waha)") for p in all_profiles]))
...
def test(ctx: typer.Context) -> None:
    ...
    if not health:
        out.error("Health endpoint returned empty response")
        raise typer.Exit(code=1)
...
def migrate(ctx: typer.Context) -> None:
    ...
    Old:  profiles.production.url -> profiles.production.waha.url
    """
    actx: AppContext = ctx.obj
    out = actx.output
