...
Usage:
    ...
    uv run python scripts/verify-backups.py --json
    uv run python scripts/verify-backups.py --dry-run
"""
...
def check_backup(config: BackupConfig) -> BackupStatus:
...
def print_dry_run(configs: list[BackupConfig]) -> None:
    ...
    for c in configs:
        ...
        interval_str = (
            f"{interval_h:.0f}h" if interval_h >= 1 else f"{interval_h * 60:.0f}m"
        )
    ...
    for c in configs:
        print(
            f"  kctl-dokploy backup list "
            f"--destination {c.destination} --prefix {c.prefix} --json"
        )
    print()
            ...
            records = [
                {
                    ...
                    "stale_after_hours": parse_cron_interval_hours(c.schedule)
