...
def add(
    ...
    """Add or update a profile's Cloudflare connection."""
    c: AppContext = ctx.obj
    out = c.output
...
def remove(
    ...
    service_only: Annotated[bool, typer.Option("--service-only", help="Only remove cloudflare config")] = False,
) -> None:
    """Remove a profile or just its Cloudflare config."""
    c: AppContext = ctx.obj
    out = c.output
    ...
    if service_only:
        if not force:
            ...
            if not typer.confirm(f"Remove {SERVICE_KEY} config from '{name}' (account: {svc.account_id})?"):
                raise typer.Exit(0)
...
def set_(
    ctx: typer.Context,
    key: Annotated[
        str, typer.Argument(help="Config key (e.g. api_token, account_id, terraform_dir, or default_profile)")
    ],
    ...
    valid_fields = {"api_token", "account_id", "terraform_dir"}
...
def profiles(ctx: typer.Context) -> None:
    """List all profiles with Cloudflare connection status."""
    c: AppContext = ctx.obj
    out = c.output

    profile_names = get_profile_names()
    if not profile_names:
        out.warn("No profiles configured. Run: kctl-cf config init")
        return
    ...
    for pname in profile_names:
        ...
        rows.append([pname, svc.account_id or "-", _mask(svc.api_token), other_str, status_marker])
        json_data.append(
            {
                "name": pname,
    ...
    out.table(
        "Profiles",
        [("Name", "cyan"), ("Account ID", ""), ("Token", ""), ("Other Services", "dim"), ("", "green")],
        rows,
...
def current(ctx: typer.Context) -> None:
    ...
    api_token, account_id = resolve_connection(
        ...
        api_token_override=c.api_token_override,
        account_id_override=c.account_id_override,
    )

    source = "config default"
    if c.profile:
        source = "--profile flag"
    elif os.environ.get("KCTL_CLOUDFLARE_PROFILE"):
        source = "KCTL_CLOUDFLARE_PROFILE env var"
    ...
    sections: list[tuple[str, list[tuple[str, str]]]] = [
        (
            ...
            [
                ...
                ("Service", SERVICE_KEY),
                ("Source", source),
                ("Account ID", account_id or "[red]not set[/red]"),
                ("API Token", _mask(api_token)),
            ],
        ),
    ]
    ...
    if all_profiles:
        sections.append(
            ("Other Profiles", [(p, get_service_config(p).account_id or "(no cloudflare)") for p in all_profiles])
        )
...
def migrate(ctx: typer.Context) -> None:
    ...
    for pname, pdata in config_profiles.items():
        if not isinstance(pdata, dict):
            continue
        if "api_token" in pdata and not any(
            isinstance(v, dict) and ("api_token" in v or "account_id" in v)
