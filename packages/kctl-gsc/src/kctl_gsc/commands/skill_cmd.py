...
def generate(
    ...
    Examples:
        kctl-gsc skill generate
        kctl-gsc skill generate --install
        kctl-gsc skill generate --check
    """
    actx: AppContext = ctx.obj
    out = actx.output
    from kctl_lib.skill_generator import check_stale, generate_skill

    from kctl_gsc.cli import app as cli_app

    skill_name = "gsc-admin"
    description = (
    ...
    if check:
        ...
        if is_stale:
            ...
            out.info("Run: kctl-gsc skill generate")
            raise typer.Exit(1)
