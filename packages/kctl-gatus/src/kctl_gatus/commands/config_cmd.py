...
from kctl_gatus.core.config import (
    CONFIG_FILE,
    SERVICE_KEY,
    ServiceConfig,
...
from kctl_gatus.core.exceptions import KctlError
...
def _test_connection(url: str, api_key: str) -> tuple[bool, str]:
    try:
        client = GatusClient(base_url=url, api_key=api_key)
...
def init(
    ctx: typer.Context,
    url: Annotated[str | None, typer.Option("--url", help="Gatus base URL.")] = None,
    api_key: Annotated[str | None, typer.Option("--api-key", help="API key (optional).")] = None,
    ...
    api_url = url or typer.prompt("Gatus URL (e.g. https://gatus.kodeme.io)")
    key = api_key
    if key is None:
        key = typer.prompt("API key (press Enter to skip — Gatus may not require auth)", default="", show_default=False)

    out.info(f"Testing connection to {api_url}...")
    ok, info = _test_connection(api_url, key)
    if ok:
    ...
    out.kv("URL", api_url)
    out.kv("API Key", _mask_token(key))
...
def add(
    ...
    """Add or update a profile's Gatus connection."""
    actx: AppContext = ctx.obj
    out = actx.output

    api_url = url or typer.prompt("Gatus URL")
    key = api_key
    if key is None:
        key = typer.prompt("API key (press Enter to skip)", default="", show_default=False)
...
def use(
    ...
    if svc.url:
        ok, info = _test_connection(svc.url, svc.api_key)
        if ok:
...
def remove(
    ...
    service_only: Annotated[bool, typer.Option("--service-only", help="Only remove gatus config")] = False,
) -> None:
    """Remove a profile or just its Gatus config."""
    actx: AppContext = ctx.obj
    out = actx.output
...
def show(ctx: typer.Context) -> None:
    """Show full configuration (keys masked)."""
    actx: AppContext = ctx.obj
    out = actx.output
            ...
            for _svc, svc_data in pdata.items():
                if isinstance(svc_data, dict):
                    if "api_key" in svc_data:
                        svc_data["api_key"] = _mask_token(svc_data["api_key"])
                    if "token" in svc_data:
                        svc_data["token"] = _mask_token(svc_data["token"])
            if "api_key" in pdata:
                pdata["api_key"] = _mask_token(pdata["api_key"])
        out.raw_json(data)
        return
    ...
    sections.append(
        (
            ...
            [
                ...
                ("This CLI", f"kctl-gatus -> service key: {SERVICE_KEY}"),
            ],
        )
    )
    ...
    for pname in profiles:
        ...
        for svc_name, svc_data in services.items():
            ...
            svc_key = _mask_token(svc_data.get("api_key", svc_data.get("token", "")))
...
def set_(
    ...
    display = _mask_token(value) if "key" in key else value
    out.success(f"[{pname}] {SERVICE_KEY}.{key} = {display}")


@app.command()
def profiles(ctx: typer.Context) -> None:
    """List all profiles with Gatus connection status."""
    actx: AppContext = ctx.obj
    out = actx.output

    profile_names = get_profile_names()
    if not profile_names:
        out.warn("No profiles configured. Run: kctl-gatus config init")
        return
    ...
    for pname in profile_names:
        ...
        if svc.url:
            ...
            conn_status = "[dim]no gatus config[/dim]"
    ...
    out.table(
        "Profiles",
        [("Name", "cyan"), ("Gatus URL", ""), ("Status", ""), ("Other Services", "dim"), ("", "green")],
        rows,
...
def current(ctx: typer.Context) -> None:
    ...
    ok, info = _test_connection(resolved_url, resolved_key) if resolved_url else (False, "not configured")

    source = "config default"
    if actx.profile:
        source = "--profile flag"
    elif os.environ.get("KCTL_GATUS_PROFILE"):
        source = "KCTL_GATUS_PROFILE env var"
    ...
    sections: list[tuple[str, list[tuple[str, str]]]] = [
        (
            ...
            [
                ...
                ("API Key", _mask_token(resolved_key)),
    ...
    if all_profiles:
        sections.append(("Other Profiles", [(p, get_service_config(p).url or "(no gatus)") for p in all_profiles]))
...
def test(ctx: typer.Context) -> None:
    ...
    try:
        c = actx.client
        status_code = c.check_health()
    except KctlError as e:
        out.error(f"Connection failed: {e}")
        raise typer.Exit(code=1) from e

    if status_code == 200:
        # Try to get some endpoint data to confirm API works
        try:
            endpoints = c.get_endpoint_statuses()
        ...
        sections = [
            (
                ...
                [
                    ...
                    ("Health", f"HTTP {status_code}"),
                    ("Endpoints", str(endpoint_count) if endpoint_count >= 0 else "unknown"),
                ],
            )
        ]
        ...
        out.error(f"Health check returned HTTP {status_code}")
        raise typer.Exit(code=1)
