"""Configuration management commands for kctl-supa.

Manage profiles and Supabase connection settings.
"""
...
from kctl_supa.core.config import (
    SERVICE_KEY,
    ServiceConfig,
...
def profiles(ctx: typer.Context) -> None:
    """List all profiles with their Supabase configuration."""
...
def show(
    ...
    out.kv("Service Role Key", _mask_secret(svc.service_role_key))
    out.kv("Anon Key", _mask_secret(svc.anon_key))
    out.kv("DB Password", _mask_secret(svc.db_password))
    ...
    out.kv("Container Prefix", svc.container_prefix or "[dim]not set[/dim]")
...
def current(ctx: typer.Context) -> None:
    ...
    if actx.profile:
        source = "--profile flag"
    elif os.environ.get("KCTL_SUPA_PROFILE"):
        source = "KCTL_SUPA_PROFILE env var"
...
def add(
    ...
    url: Annotated[str | None, typer.Option("--url", help="Supabase URL.")] = None,
    service_role_key: Annotated[str | None, typer.Option("--service-role-key", help="Service role key.")] = None,
    anon_key: Annotated[str | None, typer.Option("--anon-key", help="Anon/public key.")] = None,
    db_password: Annotated[str | None, typer.Option("--db-password", help="Database password.")] = None,
    ...
    container_prefix: Annotated[str | None, typer.Option("--container-prefix", help="Docker container prefix.")] = None,
    set_default: Annotated[bool, typer.Option("--default", help="Set as default profile.")] = False,
) -> None:
    """Add or update a Supabase profile.

    Example: kctl-supa config add production --url https://supa.example.com --ssh-host 1.2.3.4
    """
    actx: AppContext = ctx.obj
    out = actx.output
    ...
    svc = ServiceConfig(
        ...
        ssh_user=ssh_user,
        container_prefix=container_prefix or "",
    )
...
def use(
    ...
    Example: kctl-supa config use production
    """
    actx: AppContext = ctx.obj
    out = actx.output
